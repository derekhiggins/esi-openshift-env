...
sudo nmcli c s | grep -e cluster -e vlan | awk '{print \$1}' | sudo xargs -t nmcli c del

echo 'nameserver 192.168.111.1' | sudo tee /etc/resolv.conf
sudo nmcli c modify "System eth0" ipv4.dns 192.168.111.1 ipv4.ignore-auto-dns yes
...
sudo nmcli con add type bridge con-name cluster-01bm ifname cluster-01bm ipv4.method manual ipv4.address "192.168.111.1/24" ipv4.gateway "192.168.111.254" ipv4.dns 192.168.111.1 ipv4.ignore-auto-dns yes
sudo nmcli connection add type vlan con-name eth0.\$VLANID_BM dev eth0 id \$VLANID_BM ipv4.method disabled ipv6.method disabled 802-3-ethernet.cloned-mac-address \$PROV_BM_MAC master cluster-01bm
